...
def food(x, y):
    apple = pygame.transform.scale(apple_png, (20, 20))
    dis.blit(apple, (x, y))
    ...
    while intro:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                quit()
#蛇頭與食物碰撞                
def is_collision(foodx, foody, snake_head, offset):
...
def gameLoop():
    ...
    while not game_over: 
        ...
        #食物
        dis.blit(apple_png, (foodx,foody))
        #分數
        ...
        #吃到食物後長長
        if is_collision(foodx, foody, snake_Head,20):
            sound1.play()
            foodx = round(random.randrange(0, dis_width - snake_block) / 10.0) * 10.0
            foody = round(random.randrange(0, dis_height - snake_block) / 10.0) * 10.0
            Length_of_snake += 1
